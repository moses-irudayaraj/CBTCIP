...
# Generate a random player number between 1 and 10
...
# Determine the opponent based on the initially chosen player
if player % 2 == 0:
    opponent = 1
    player = 1
else:
...
for curr in players.keys():
    ...
    while isTrue:
        ...
        if g_p == len(pl_num):
            ...
            if pl_ts == 1:
                print(f"\n Congrats Mastermind! player{curr} you won in the first attempt itself!!!")
                exit(0)
            isTrue = False
        else:
            print('\n\n try again')
